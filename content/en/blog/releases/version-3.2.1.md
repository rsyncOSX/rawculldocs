...
RawCull 3.2.1 is the modular-AI release based on RawCull 3.1.1. It is available through Apple's TestFlight; please email thomeven@gmail.com if you would like to try that build.

<div class="alert alert-secondary" role="alert">

RawCull's AI implementation is organized behind focused feature boundaries for similarity, semantic search, burst analysis, Deep Review, model management, persistence, and presentation. DataComp CLIP powers similarity and semantic search, while SAM 3 powers Deep Review. The 3.2.1 release also adds managed downloads for the verified CLIP and SAM 3 model packs.

</div>

RawCull 3.2.1 requires the latest macOS 27 (Golden Gate), an Apple Silicon Mac, and Xcode 27 when building from source.

**Commit range:** `3b1c41d..3caf6ba`  
**Latest commit:** `3caf6ba` (`Upgrade PhotoAIKit and CoreAI (#11)`, 2026-09-10)  
**Current version:** RawCull 3.2.1 (Build 360)

## 🚀 Major Features

- Released the RawCull 3 local AI workflow.
...
- Added SAM 3 Deep Review with subject isolation, detail scoring, and burst-winner recommendations.
- Added managed downloads for verified DataComp CLIP and SAM 3 model packs.
...
- Split model discovery, downloads, license acceptance, progress, cancellation, retry, removal, and installed-model locations into a focused model-management component.
...
- Reorganized the AI source files under `RawCull/Intelligence` into Composition, Contracts, Similarity, SemanticSearch, BurstAnalysis, DeepReview, ModelManagement, Persistence, and Presentation areas.
...
- Published the V3 managed model release containing DataComp CLIP and SAM 3 asset packs.
- Added archive size and SHA-256 verification for the published CLIP and SAM 3 downloads.
- Added provenance validation that rejects incomplete, mismatched, blocked, or malformed production model records.
- Added EfficientSAM license and provenance records for future use; EfficientSAM is not included in the production download manifest.
...
- Updated PhotoAIKit to v2.4.0 and the matching `apple/coreai-models` revision.
- Updated the resolved `xgrammar` dependency to its released 0.2.2 version.
- Retained the V3 compiled DataComp CLIP and SAM 3 assets. A V4 model release is not required unless a future CoreAI export changes the verified model artifacts or model contract.
...
- Added Deep Review availability indicators and overlay controls to thumbnail presentation.
...
- Added keyboard actions for the zoom overlay.
- Added optional focus-mask and subject-outline presentation to the zoom workflow.
- Preserved native preview detail for focus-mask analysis instead of using sharpened thumbnail output.

## 🎞️ Burst View and Deep Review
...
- Added SAM 3 subject-mask outlines for Deep Review and focus workflows.
- Added mask-outline controls and availability feedback.
- Fixed focus-mask source selection for burst JPEG previews.
...
## 🗂️ Sidebar, Navigation, and File Operations
...
- Improved copy-file startup, completion reporting, source and destination presentation, and recovery after interrupted work.
- Improved JPEG extraction selection handling and per-file failure reporting.
...
- Reduced `CullingGridView` rendering work identified during performance tracing.
- Reduced image-downscaling work identified during performance tracing.
...
- Used PhotoAnalysisKit focus-mask morphology defaults and removed obsolete RawCull visibility overrides.
...
- Added accessible state and controls for Deep Review mask availability and subject outlines.
...
- Added provenance-validation tests covering malformed records, missing evidence, archive mismatches, blocked distributions, and invalid notice hashes.
...
- Added tests for comparison-image source selection, focus-mask resolution policy, subject-mask outline rendering, zoom-overlay keyboard actions, copy completion, and quit recovery.
...
- Built RawCull successfully after resolving the PhotoAIKit v2.4.0 and CoreAI dependency upgrade.
...
- Removed superseded modular-AI planning documents after consolidating the active model-release and provenance guidance.

## 🛠️ Build and Documentation

- Updated the project for the RawCull 3.2.1 release.
- Advanced RawCull to Build 360.
...
- Updated PhotoAIKit to v2.4.0 and CoreAI models to revision `cc8120`.
...
- Updated the README with the version-3.2.1 release requirements, AI workflow, model-download guidance, and build instructions.
- Updated export, release, smoke-test, performance-test, import-boundary, and model-provenance verification.
...
- Published the V3 DataComp CLIP and SAM 3 managed model packs.
- Added Deep Review mask outlines, focus-mask improvements, copy-file reliability fixes, and performance improvements.
- Updated RawCull to PhotoAIKit v2.4.0 and its matching CoreAI revision.
- Released RawCull 3.2.1, Build 360.
- Updated this changelog through commit `3caf6ba` on 2026-09-10.